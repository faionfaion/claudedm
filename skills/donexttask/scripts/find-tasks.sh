...
find $HOME/aidocs/sdd/*/features/*/tasks/in_progress/ -name "TASK_*.md" 2>/dev/null | while read f; do
...
echo ""
echo "=== FEATURE TODO ==="
find $HOME/aidocs/sdd/*/features/*/tasks/todo/ -name "TASK_*.md" 2>/dev/null | while read f; do
  feature=$(echo "$f" | sed 's|.*/sdd/\([^/]*\)/features/\([^/]*\)/.*|\1/\2|')
  echo "[$feature] $f"
done
